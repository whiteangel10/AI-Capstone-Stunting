...
df_stunting = pd.read_csv("data/stunting_dataset4.csv")
...
def generate_answer_stunting(question, user_id, threshold=2.5):
    if not is_relevant_to_stunting(question):
        return "Maaf, pertanyaan Anda tidak berkaitan dengan topik stunting atau MPASI. Anda bisa mencoba mengajukan pertanyaannya kembali ke admin", "belum terjawab"
...
def generate_answer_mpasi(question, usia_bulan, user_id=None, threshold=2.5):
    ...
    if filtered_df.empty:
        return f"Maaf, tidak ada rekomendasi MPASI untuk usia {usia_bulan} bulan.", "belum ada rekomendasi"
